 ...
 *  one per thing we build, telling one customer's journey from top to bottom:
 *  Priya asks for an AC installation on a website, WhatsApp finds out what she needs, n8n
 *  prices it, the quote is accepted, and the warranty is issued.
 *
...
export type LayerId = 'web' | 'wa' | 'n8n' | 'quote' | 'warranty';
...
export const LAYERS: Layer[] = [
  { id: 'web',      num: '01', name: 'Static website',      spec: 'Loads in 0.9 s · score 99',   chip: 'Website',    log: 'site: quote request sent' },
  { id: 'wa',       num: '02', name: 'WhatsApp automation', spec: 'Replies in seconds, 24/7',    chip: 'WhatsApp',   log: 'whatsapp: room size asked ✓✓' },
  { id: 'n8n',      num: '03', name: 'n8n automation',      spec: '4 steps, no copy-paste',      chip: 'Automation', log: 'n8n: priced and PDF made' },
  { id: 'quote',    num: '04', name: 'Quotation system',    spec: 'Sent, opened, accepted',      chip: 'Quote',      log: 'quote: Q-2041 accepted' },
  { id: 'warranty', num: '05', name: 'Warranty system',     spec: 'QR check · valid to 2027',    chip: 'Warranty',   log: 'warranty: issued, valid 2027' },
];
...
export const SLOTS = LAYERS.map((_, i) => ({ x: 376 - i * 6, y: 118 + i * 100 }));
...
export const ISO_SCALE = 0.84;

/** Scroll story (desktop and tablet): the pin, and the windows on its 0..1 timeline. */
export const PIN_END = '+=220%';
export const RUN = { start: 0.27, each: 0.115 } as const;   // 5 windows: 0.27 -> 0.845
