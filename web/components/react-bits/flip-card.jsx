'use client';
/* FlipCard — React Bits (JS-CSS variant): https://reactbits.dev/micro/flip-card
 * Registry: https://reactbits.dev/r/FlipCard-JS-CSS.json   Dependency: motion
 *
 * Vendored unchanged except for two props, marked `LF:` below:
 *  - reduceMotion   OR'd with the OS setting, so the site's footer "Animations" switch
 *                   (PLAN §5.5) turns the 3D flip into a fade too.
 *  - presentational when an external control (our name-tag buttons) is the accessible
 *                   interface, the card itself leaves the tab order and the a11y tree;
 *                   pointer interaction still works. */
...
export default function FlipCard({
  ...
  ariaLabel = 'Flip card',
  className = '',
  reduceMotion = false, // LF
  presentational = false // LF
}) {
  const reduce = useReducedMotion() || reduceMotion; // LF
  ...
  return (
    <div
      ref={rootRef}
      {...(presentational // LF: name tags are the control; keep the visual out of the a11y tree
        ? { 'aria-hidden': true, tabIndex: -1 }
        : { role: 'button', tabIndex: disabled ? -1 : 0, 'aria-pressed': shown, 'aria-label': ariaLabel })}
      ...
      onKeyDown={presentational ? undefined : onKeyDown} // LF
      onClick={onClick}
