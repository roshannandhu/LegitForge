'use client';
/* LogoLoop — React Bits (JavaScript + CSS variant), vendored from the owner's copy of the
 * registry source (reactbits.dev is unreachable from the build sandbox). Same API and behaviour.
 * LF: additions, keep them if you swap in the official file:
 *  - paused       stops the animation frame loop entirely (off screen, or motion switched off),
 *                 so a budget phone never spends a frame on a strip it cannot see
 *  - 'use client' and the CSS import path */
...
const useAnimationLoop = (trackRef, targetVelocity, seqWidth, seqHeight, isHovered, hoverSpeed, isVertical, paused) => {
  ...
  useEffect(() => {
    ...
    if (paused) return;                                              // LF: no frames at all while paused
...
export const LogoLoop = memo(
  ({
    ...
    useAnimationLoop(trackRef, targetVelocity, seqWidth, seqHeight, isHovered, effectiveHoverSpeed, isVertical, paused);
    ...
    const renderLogoItem = useCallback(
      (item, key) => {
        ...
        const content = isNodeItem ? (
          ...
          // eslint-disable-next-line @next/next/no-img-element -- the component's own image path
