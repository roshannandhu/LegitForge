/** POST /api/admin/upload — one image for a project, or a team member's photo (PLAN §7.8). Paste or file pick in the
...
import { getMember, getProjectRow, insertImage, setMemberPhoto } from '@/lib/admin/db';
...
export async function POST(req: Request) {
  ...
  if (memberId ? !(await getMember(memberId)) : !(await getProjectRow(projectId))) return json({ error: 'That project or person doesn’t exist.' }, 404);
  ...
  const key = memberId ? `team/${memberId}/${hash.slice(0, 32)}.${TYPES[file.type]}` : `projects/${projectId}/${hash.slice(0, 32)}.${TYPES[file.type]}`;
  await env.MEDIA.put(key, bytes, { httpMetadata: { contentType: file.type, cacheControl: 'public, max-age=31536000, immutable' } });

  if (memberId) {                                        // a team photo: replaces the old one and redraws the card
    const old = await setMemberPhoto(memberId, key);
    if (old && old !== key) await env.MEDIA.delete(old);
    revalidateTag('team', { expire: 0 });
    return json({ ok: true, key, width, height });
  }
